...
/**
 * CS2 饰品标题区：Steam `priceoverview` + **`search/render` 的 `icon_url` 图**（公开字段）。
 */
export default function CsgoItemHeader({
  ...
  const actions = [
    { key: "compare", icon: BarChart2Icon, label: t("stockHeader.compare") },
    { key: "share", icon: Share2Icon, label: t("stockHeader.share") },
    { key: "bookmark", icon: BookmarkIcon, label: t("stockHeader.bookmark") },
    { key: "watchlist", icon: PlusIcon, label: t("stockHeader.addWatchlist") },
  ] as const;
  ...
  return (
    <div data-cmp="CsgoItemHeader" className="relative border-b border-border bg-panel px-4 py-2.5">
      ...
      <div className="flex flex-wrap items-start gap-3">
        <CsgoItemIcon src={quote?.iconUrl} alt={marketHashName} size="lg" />
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-1">
          ...
          <div className="text-xs text-muted-foreground">{t("csgo.dataSource")}</div>
          {quote?.dataSource === "demo" ? (
            <div className="rounded bg-amber-500/15 px-2 py-1 text-xs text-amber-700 dark:text-amber-400">
              {t("csgo.demoDataBanner")}
            </div>
          ) : null}
        </div>

        <div className="flex shrink-0 items-end gap-3">
          <div className="font-mono text-3xl font-bold leading-none text-foreground">
            {price != null ? price.toFixed(2) : `—.——`}
          </div>
          <div className="text-muted-foreground flex flex-col items-start pb-0.5 text-xs">
            <span>{quote?.lowestRaw ?? dash}</span>
          </div>
        </div>

        <button
          type="button"
          className="mt-1 shrink-0 p-1 text-muted-foreground transition-colors hover:text-chart-3"
        >
          <StarIcon size={14} />
        </button>
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-1">
          {actions.map(({ key, icon: Icon, label }) => (
            <button
              key={key}
              type="button"
              disabled={key === "share" && !marketHashName.trim()}
              className="flex items-center gap-1 rounded px-2 py-1 text-xs text-muted-foreground transition-colors hover:bg-muted hover:text-foreground disabled:pointer-events-none disabled:opacity-40"
              onClick={key === "share" ? () => void onShare() : undefined}
            >
              <Icon size={12} />
              <span>{label}</span>
            </button>
